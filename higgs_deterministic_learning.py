...
parser.add_argument("--predict-only", action="store_true",
                        help="Load model artifacts JSON and write predictions for --data-file.")
parser.add_argument("--model-artifact", type=str, default="",
                        help="Path to saved JSON artifacts (from a previous training run).")
parser.add_argument("--predictions-out", type=str, default="predictions.csv",
                        help="Where to write predictions when --predict-only is used.")
parser.add_argument('--method', type=str, default='projector',
                    choices=['projector','jumps','boson'],
                    help='Deterministic method to use in predict-only mode.')
parser.add_argument('--importance-mode', type=str, default='lambda',
...
# =============================================================================
# BASE PREDICTOR MIXIN
# =============================================================================

class _BasePredictor:
    """Mixin for deterministic label prediction using predict_proba + threshold."""
    def predict(self, X, threshold=None):
        proba = self.predict_proba(X)
        th = getattr(self, "theta_star_", None) if threshold is None else threshold
        if th is None:
            th = 0.5
        return (proba >= float(th)).astype(int)
...
class SingleStepProjector(_BasePredictor):
    """
...
class AdaptiveGeometricJumps(_BasePredictor):
    """
...
class GBInformedBosonFold(_BasePredictor):
    """
...
def main():
    ...
    proj.theta_star_ = float(theta_proj)  # Store for predict() method
    ...
    jumps.theta_star_ = float(theta_jumps)  # Store for predict() method
    ...
    boson.theta_star_ = float(theta_boson)  # Store for predict() method
